...
class EmergencyService {
  static const String defaultEmergencyNumber = "9148337532";
  ...
  static Future<void> triggerEmergencyProtocol(String triggerType, {List<String>? emergencyContacts}) async {
    ...
    try {
      position = await Geolocator.getCurrentPosition(
          ...
          timeLimit: const Duration(seconds: 3)); // Max 3 seconds to find GPS
    } catch (e) {
      print("GPS unavailable or timed out, proceeding without precise location.");
    }

    // 2. Notify Backend (Logs Event)
    // DO NOT AWAIT THIS! The backend network failing should never delay an emergency call.
    _notifyBackend(position, triggerType);
    ...
    // 4. Initiate the Phone Call
    // Dial the first emergency contact, otherwise fallback to Cariappa
    if (emergencyContacts != null && emergencyContacts.isNotEmpty) {
      await callNumber(emergencyContacts.first);
    } else {
      await callNumber(defaultEmergencyNumber);
    }
  }
