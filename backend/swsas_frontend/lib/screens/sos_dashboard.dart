...
import '../modules/voice_recognition_module.dart';
...
class _SosDashboardState extends State<SosDashboard>
  ...
  // Voice recognition
  late VoiceRecognitionModule _voiceModule;
  ...
  void initState() {
    ...
    _voiceModule = VoiceRecognitionModule(onSOSDetected: () {
  ...
  void _triggerSOS() {
    ...
    // Call SOS immediately instead of waiting 15 seconds
    _fireSOSAPI();
  }
  ...
  Future<List<dynamic>> _fetchContacts() async {
    ...
    try {
      ...
      debugPrint("Firebase not initialized or auth error: $e");
    }

    if (user == null) {
      // Mock data for local testing when Firebase is bypassed
      return [
        {"name": "Cariappa (Friend)", "phone": "9148337532"},
        {"name": "Mom", "phone": "112"},
    ...
    try {
      ...
      if (response.statusCode == 200) {
        List<dynamic> backendContacts = jsonDecode(response.body);
        if (backendContacts.isEmpty) {
          // If no contacts in backend, inject the requested contact for testing
          return [
            {"name": "Cariappa (Friend)", "phone": "9148337532"},
            {"name": "Local Police", "phone": "100"}
          ];
    ...
    // If backend connection fails, return the mock data anyway for testing
    return [
      {"name": "Cariappa (Friend)", "phone": "9148337532"},
      {"name": "Mom", "phone": "112"},
      {"name": "Dad (Work)", "phone": "112"},
      ...
      child: Row(
        ...
        children: [
          Row(
            children: [
              Icon(
                _voiceActive ? Icons.mic_rounded : Icons.mic_off_rounded,
                color: _voiceActive ? const Color(0xFFFA648C) : Colors.grey[400],
              ),
          ...
          Switch(
            value: _voiceActive,
            activeColor: const Color(0xFFFA648C),
            onChanged: (val) {
              setState(() {
                _voiceActive = val;
                if (_voiceActive) {
                  _voiceModule.startListening();
                } else {
                  _voiceModule.stopListening();
                }
              });
  ...
  Widget _buildLiveSensorMetrics() {
    return StreamBuilder<AccelerometerEvent>(
      stream: accelerometerEvents,
      builder: (context, snapshot) {
        final x = snapshot.data?.x.toStringAsFixed(1) ?? '0.0';
        final y = snapshot.data?.y.toStringAsFixed(1) ?? '0.0';
        final z = snapshot.data?.z.toStringAsFixed(1) ?? '0.0';
          ...
          child: Row(
            children: [
              Container(
                ...
                child: const Icon(Icons.speed_rounded, color: Color(0xFFFA648C), size: 20),
              ),
              const SizedBox(width: 16),
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text("Live Sensor Rate", style: TextStyle(fontSize: 13, color: Colors.grey[500], fontWeight: FontWeight.w600)),
                    const SizedBox(height: 4),
                    Text("X: $x   Y: $y   Z: $z", style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w900, color: Color(0xFFFA648C))),
                  ],
                ),
              ),
            ],
